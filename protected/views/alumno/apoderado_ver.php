...
<div class="row">
	<div class="span10 text-center">
		<h3><?php echo $notas_1[0]->notMat->matAlu->getNombre_completo(); ?></h3>
			<p class="text-info"><strong>Año Escolar  <?php echo $ano; ?></strong>  </p>
	</div>
</div>
...
<tbody>
    ...
    <tr>
		...
		<?php for ($i=1; $i <= $notas_curso; $i++) { ?>
			...
			<?php if($i < 10){ ?>
				<td id="nota"><?php if(strlen($key['not_0'.$i]) == 1 AND $key['not_0'.$i] > 0 )
										echo $key['not_0'.$i].".0";
									else{
										echo $key['not_0'.$i];
									}
			...
			<?php
				...
				<td id="nota"><?php if(strlen($key['not_'.$i]) == 1 AND $key['not_'.$i] > 0 )
										echo $key['not_'.$i].".0";
									else{
										echo $key['not_'.$i];
									} ?></td>
